...
/**
 * This sample demonstrates how to access the Luna SIM interface
 * through Java.
 * <p>
 * <b>This sample application will not run on SA 5.0 since it
 * does not support theSIM capability.</b>
 */
public class SIMExample {
    ...
    public boolean simWithPassword() {
        ...
        try {
            ...
            System.out
            .println("Unable to create a SIM key from the given key.");
            return false;
        }
        ...
        try {
            ...
            System.out
            .println("Invalid Key Exception during UnSIM operation without password.");
            return false;
        } catch (LunaCryptokiException lce) {
            if (lce.getMessage().contains("80000017"))
                System.out
                .println("UnSIM operation correctly failed with Cryptoki error CKR_SIM_AUTHORIZATION_FAILED");
        }
        ...
        try {
            ...
            System.out
            .println("Invalid Key Exception during UnSIM operation with a bad password.");
            return false;
        } catch (LunaCryptokiException lce) {
            if (lce.getMessage().contains("80000017"))
                System.out
    ...
    public boolean simMofNPasswords() {
        ...
        try {
            ...
            privKey = (LunaPrivateKeyRsa) simKey.getKey(
                    LunaAPI.CKA_SIM_PASSWORD, pwList);
        } catch (InvalidKeyException ike) {
            System.out
            .println("Invalid Key Exception during UnSIM operation with < M passwords.");
            return false;
        } catch (LunaCryptokiException lce) {
            if (lce.getMessage().contains("80000017"))
                System.out
        ...
        try {
            ...
            String[] pwArray = { "These", "are", "passwords", "for", "M", "of",
                    "N", "SIM", "encryption" };
            List<String> pwList = Arrays.asList(pwArray);
            privKey = (LunaPrivateKeyRsa) simKey.getKey(
                    LunaAPI.CKA_SIM_PASSWORD, pwList);
    ...
    private boolean multiKeyExample() {
        ...
        // SIM TYPES:
        /*
         * CKA_SIM_NO_AUTHORIZATION // no authorization needed CKA_SIM_PASSWORD
         * // plain-text passwords CKA_SIM_CHALLENGE // challenge secrets
         * emitted through the secure port CKA_SIM_SECURE_PORT // PED keys
         * CKA_SIM_PORTABLE_NO_AUTHORIZATION // no authorization needed,
         * portable CKA_SIM_PORTABLE_PASSWORD // plain-text passwords, portable
         * CKA_SIM_PORTABLE_CHALLENGE // challenge secrets emitted through the
         * secure port, portable CKA_SIM_PORTABLE_SECURE_PORT // PED keys,
         * portable
         */
        ...
        System.out
        .println("This test is designed to use a second partition / slot");
        System.out.println("It uses Portable SIM.");
        System.out
        .println("If the target slot does not use the same domain as the");
        ...
        try {
            ...
            System.out
            .println("Unexpected InvalidKeyException while SIM Extracting keys");
            System.exit(-1);
        }
        ...
        System.out
        .println("Which slot would you like to use for SIM Insertion?");
        System.out.println("Slot 1 was used for extraction. \n");
        System.out.println(" Choice: [1-"
                + LunaSlotManager.getInstance().getNumberOfSlots() + "]");
        ...
        try {
            if (slot != 1) {
                System.out.println("Please enter the password for slot: "
                        + slot);
                input = br.readLine();
        ...
        try {
            ...
            System.out.println("SIM INSERT operation inserted " + keys.length
                    + " total keys");
    ...
    /**
     * Helper method to sign/verify some data using the public and private keys
     * given
     */
