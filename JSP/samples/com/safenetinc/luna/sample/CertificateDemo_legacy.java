...
public class CertificateDemo_legacy {
    /*
     * This example illustrates - Generate an RSA KeyPair using the default
     * provider - Self-Sign with an RSA key - Store a Certificate - Store Keys
     * 
     * For more information regarding Luna KeyStores please see
     * KeystoreLunaDemo.java.
     */

    /*
     * 
     * NOTICE: THIS IS FOR LEGACY PURPOSES ONLY. PLEASE USE THE OTHER METHOD
     * PRESENTED IN CertificateDemo.java ONLY
     * 
     * The Luna Java Provider gives you an alternative to using keystores for
     * storing keys and single certificate. There are some special methods in
     * our LunaKey and LunaCertificateX509 classes that give you direct access
     * to storing, retrieving, and removing objects from the HSM. The LunaKey
     * methods also permit you do store public key objects, which KeyStores do
     * not.
     * 
     * In this example we will generate an RSA KeyPair, self-sign a certificate,
     * and save the certificate and key pair on the HSM.
     */
    public static void main(String[] args) {
        ...
        try {
            ...
            /*
             * Even though the RSA KeyPair created above was created with the
             * Luna Provider, the keys are not permanently stored on the
             * Luna HSM automatically. Keys are made persistent (or in
             * PKCS#11 terms, become token objects instead of session objects)
             * only when they are saved in a KeyStore, or when the special
             * LunaKey MakePersistent() method is invoked.
             */
        ...
        try {
            ...
            certChain[0] = LunaCertificateX509.SelfSign(RSAKeyPair,
                    subjectname, serialNumber, notBefore, notAfter);
        ...
        try {
            ...
            /*
             * Even though the certificate created above was signed with a key
             * that is stored in the Luna HSM, the certificate is not
             * permanently stored on the Luna HSM automatically. Certificates
             * are made persistent (or in PKCS#11 terms, become token objects
             * instead of session objects) only when they are saved in a
             * KeyStore, or when the special LunaCertificateX509
